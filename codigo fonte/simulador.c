...
fprintf(fp, "%ld %d CRIAÇÃO\n", relogio, novos->data[0]->pid);
...
fprintf(fp, "%ld %d TERMINO\n", relogio, executando->pid);
fprintf(fpterm, "%d ", executando->pid);	
...
fprintf(fp, "%ld %d DESBLOQUEIO\n", relogio, bloqueados->data[i]->pid);
		...
		if(!executando){
			...
			if(executando != NULL){
if (auxiliar != executando)
{
fprintf(fp, "%ld %d EXEC\n", relogio, executando->pid);
...
/*
	...
	fseek(fpterm, 0, SEEK_END);
	int filesize;
	filesize = ftell(fpterm);
	rewind(fpterm);
	char termino[filesize];
	termino[filesize-1] = '\0';
	fgets(termino, filesize, fpterm);	
	fprintf(saida,"CHAVEAMENTOS: %ld\n"
			"TME: %.2f\n"
			"TMR: %.2f\n"
			"VAZAO: %f\n"
			"TERMINO: %s\n"
			"DIAGRAMA DE EVENTOS\n", trocas_de_contexto, TME, TMR, vazao, termino);
		char buffer[256];
	rewind(fp);	
		while(!feof(fp))
		{
